...
public class RunTimePermissions {
    ...
    private static final int MY_PERMISSION_ACCESS_PHONE_CALL = 60;
    ...
    public void checkPhoneCallAccess() {
        int permissionCheck = ContextCompat.checkSelfPermission(applicationContext, Manifest.permission.CALL_PHONE);

        if(permissionCheck != PackageManager.PERMISSION_GRANTED) {
            ActivityCompat.requestPermissions(currentActivity, new String[] {Manifest.permission.CALL_PHONE}, MY_PERMISSION_ACCESS_PHONE_CALL);
        }
    }
    ...
    public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        ...
        switch (requestCode) {
            ...
            case MY_PERMISSION_ACCESS_PHONE_CALL: {

                if(grantResults.length > 0 && (grantResults[0] == PackageManager.PERMISSION_GRANTED)) {
                    Log.i(TAG, "Phone Call Access permission granted");
                } else {
                    // Disable any application functionalities that are dependent on the above permission.
                }
