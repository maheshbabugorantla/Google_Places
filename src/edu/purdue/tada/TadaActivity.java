...
public class TadaActivity extends BaseActivity{
		...
		try {
            ...
            if ((unsentList.size()) != 0) {
    			unsent.setText(String.format("%d", unsentList.size()) + " unsent event(s)");
		...
		unsent.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				String filepath = unsentList.get(0);//save the first line as "filepath", which will be deleted later.
				
				String gpspath = filepath.substring(0, filepath.lastIndexOf("."))+".gps";
				...
				if (unsentFile.exists()) {
					...
					if (isNetworkConnected()) {
						...
						//read the fourth and fifth lines in the rec file into singleton.
						try {
							FileInputStream fis = null;
						...
						//read the third and fourth lines in the gps file into singleton.
						try {
							FileInputStream fis = null;
							...
							ActivityBridge.getInstance().setLongitude2(data[2].substring(0, data[2].lastIndexOf(",")));
							ActivityBridge.getInstance().setLatitude2(data[2].substring(data[2].lastIndexOf(",")+1));
							//update longitude and latitude of the second image
							ActivityBridge.getInstance().setLongitude1(data[3].substring(0, data[3].lastIndexOf(",")));
							ActivityBridge.getInstance().setLatitude1(data[3].substring(data[3].lastIndexOf(",")+1));
						
						} catch (Exception e) {
							e.printStackTrace();
						}
						//rewrite unsentRec.txt: delete the first line, update the list
						try {
					        ...
					        //delete the first item in the list, namely delete "filepath"
					        ...
					        fos.close();
					        //display a message showing that txt file is updated 
					        Toast.makeText(getApplicationContext(), "TXT file is updated!", 1000).show();						
						} catch (Exception e) {
							e.printStackTrace();
						}
						//after everything is saved in singleton, call httpssendimage to upload images to the server
						Intent intent = new Intent();
						intent.setClass(TadaActivity.this,HttpsSendImage.class);
						startActivityForResult(intent, UPLOAD_UNSENT);
					}else {
						Toast.makeText(getApplicationContext(), "No Internet!", 500).show();
					}
				}else {
		...
		img_before. setOnClickListener(new View.OnClickListener(){
			@Override
			public void onClick(View v) {
				System.out.println("0:::"+ActivityBridge.getInstance().getImgFlag());
				if (ActivityBridge.getInstance().getImgFlag() == 0) {
					...
					//create a dialog to warn users if they want to replace the first image 
					AlertDialog.Builder builder = new AlertDialog.Builder(TadaActivity.this);
